...
def create_logger(logger_name: str,
    ...
    """
    # Input validation
    if logging_level not in allowed_logging_values.__args__:
        raise AttributeError("Argument passed to `logging_level` must be one of ['debug','error','critical','info',"
                             "'warning']")

    # Assume we aren't in an interactive environment until proven otherwise
    interactive = False
    try:
    ...
    # If we are in an interactive environment (like jupyter), set level
